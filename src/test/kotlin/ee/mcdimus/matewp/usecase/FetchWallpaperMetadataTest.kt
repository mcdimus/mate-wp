...
class FetchWallpaperMetadataTest : BehaviorSpec({
    ...
    val useCase = FetchWallpaperMetadata(httpClient, RetryRegistry.of(retryConfig))
    ...
    Given("FetchWallpaperMetadata use-case") {
        ...
        When("is executed and resource is not found") {
            ...
            Then("failure is returned") {
                result shouldBe Failure(
                    message = "failed to fetch: status 404",
                    cause = IllegalStateException("status 404")
                )
            }
        }
        ...
        When("is executed and timeout is reached") {
            ...
            Then("failure is returned") {
                result shouldBe Failure(
                    message = "failed to fetch: timeout reached",
                    cause = HttpTimeoutException("timeout reached")
                )
                verify(exactly = 3) { httpClient.send(any(), any<HttpResponse.BodyHandler<String>>()) }
            }
        }
